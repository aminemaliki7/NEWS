...
document.getElementById("ssmlBtn").addEventListener("click", function () {
    // Toggle tab buttons
    document.querySelectorAll(".article-content-options .btn").forEach(btn => btn.classList.remove("active"));
    this.classList.add("active");

    // Toggle content views
    document.querySelectorAll(".modal-article-content").forEach(div => div.classList.add("d-none"));
    document.getElementById("ssmlContainer").classList.remove("d-none");
});

document.addEventListener("DOMContentLoaded", function () {
    const btns = document.querySelectorAll(".article-content-options .btn");
    const contents = {
        fullContentBtn: "fullContent",
        summaryBtn: "summaryContent",
        voiceOptimizedBtn: "voiceOptimizedContent",
        youtubeScriptBtn: "youtubeScriptContent",
        ssmlBtn: "ssmlContainer"
    };
    ...
    btns.forEach(btn => {
        btn.addEventListener("click", function () {
            ...
            const target = contents[this.id];
            if (target) document.getElementById(target).classList.remove("d-none");
        });
    });
});
document.getElementById("generateVoiceFromSSML").addEventListener("click", function () {
    const ssmlText = document.getElementById("ssmlContent").value.trim();
    ...
    if (!ssmlText) {
        alert("Please enter SSML content.");
        return;
    }

    // Create a form to POST SSML
    const form = document.createElement("form");
    form.method = "POST";
    form.action = "/upload-ssml";  // Your Flask endpoint for SSML
    form.style.display = "none";
    ...
    addField("ssml", ssmlText);
    addField("voice", voiceId);
    addField("speed", speed);
    addField("depth", depth);
    addField("title", title);
    addField("input-method", "ssml");

    document.body.appendChild(form);
    form.submit();
});
const ssmlBtn = document.getElementById('ssmlBtn');
const ssmlContainer = document.getElementById('ssmlContainer');
const ssmlContent = document.getElementById('ssmlContent');

if (ssmlBtn && ssmlContainer && ssmlContent) {
    ssmlBtn.addEventListener("click", function () {
        document.querySelectorAll(".article-content-options .btn").forEach(btn => btn.classList.remove("active"));
        this.classList.add("active");

        document.querySelectorAll(".modal-article-content").forEach(div => div.classList.add("d-none"));
        ssmlContainer.classList.remove("d-none");
    });
}
